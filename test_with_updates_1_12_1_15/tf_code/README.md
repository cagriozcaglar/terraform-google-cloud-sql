...
This module handles the deployment of a Google Cloud SQL instance, including its databases and users.

It is designed with security and production-readiness in mind, providing sensible defaults such as enabled deletion protection, IAM database authentication, private networking, and encrypted connections. It supports PostgreSQL, MySQL, and SQL Server database engines.

## Usage

Below is a basic example of how to use the module to create a PostgreSQL instance with a private IP address, a database, and a user.

```hcl
module "sql" {
  source = "./" // Replace with the module source, e.g., a Git URL

  project_id       = "your-gcp-project-id"
  name             = "my-app-db-instance-1"
  database_version = "POSTGRES_14"
  region           = "us-central1"
  tier             = "db-n1-standard-2"
  disk_size        = 20

  // Configure networking for private access.
  // Public IP is disabled by default.
  ip_configuration = {
    private_network = "projects/your-gcp-project-id/global/networks/your-vpc-name"
  }

  // Define databases to be created.
  databases = [
    {
  ...
  // In a production environment, use a secrets manager to handle the password.
  users = [
    {
      name     = "my_app_user"
  ...
  // Optional: Customize backup schedule
